...
export class Board {
    ...
    private getPieceAtStartingPosition(x:number, y:number): Piece|undefined {
        if (y===1) {
            ...
            switch(x) {
                ...
                case 3:
                    return this.pieces[color]['q'][0];
                case 4:
                    return this.pieces[color]['k'][0];
            }
        }
    }
    ...
    isInCheck(color: Color, state: GameState): boolean {
        for (const cell of this.allCells) {
            if (cell.piece && cell.piece.color !== color) {
                ...
                const takesKing = moves.find((c: Move) => c.takenPiece?.name === 'k');
                if (takesKing) {
                    return true;
                }
            }
    ...
    whosIsCheck(state: GameState): Color | undefined {
        if (this.isInCheck(Color.White, state)) {
            return Color.White;
        } else if (this.isInCheck(Color.Black, state)) {
            return Color.Black;
        } else {
            return undefined;
        }
